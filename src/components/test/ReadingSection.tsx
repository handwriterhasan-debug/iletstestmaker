...
export default function ReadingSection({ onComplete, timeRemaining, isPractice, passage: propPassage, testSet }: Props) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-[calc(100vh-280px)]">
      {/* Passage Panel */}
      <div className="lg:w-1/2 glass-card p-10 overflow-y-auto custom-scrollbar bg-white/[0.02]">
      ...
      {/* Question Panel */}
      <div className="lg:w-1/2 glass-card p-8 overflow-y-auto custom-scrollbar">
              ...
              {q.type === 'tfng' ? (
                ...
                <div className="space-y-2">
                  <input 
                    ...
                    disabled={submitted}
                    className={`w-full bg-black/5 dark:bg-white/5 border rounded-xl py-4 px-6 focus:outline-none transition-colors ${submitted ? (isCorrect(q.id) ? 'border-green-500' : 'border-red-500') : 'border-black/10 dark:border-white/10 focus:border-green-500'}`}
